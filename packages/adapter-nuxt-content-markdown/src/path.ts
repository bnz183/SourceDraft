...
import {
  resolveNuxtContentMarkdownOptions,
  type FilenameConvention,
} from "./options.js";

export type NuxtContentMarkdownPathConfig = {
  contentDir: string;
  extension?: string;
...
export function getNuxtContentMarkdownPath(
  article: Article,
  ...
  const extension = rawExtension.startsWith(".")
    ...
    : rawExtension;
  const options = resolveNuxtContentMarkdownOptions(config.adapterOptions);
