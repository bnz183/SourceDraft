...
import {
  resolveMkdocsMarkdownOptions,
  type FilenameConvention,
} from "./options.js";

export type MkdocsMarkdownPathConfig = {
  contentDir: string;
  extension?: string;
...
export function getMkdocsMarkdownPath(
  article: Article,
  ...
  const extension = rawExtension.startsWith(".")
    ...
    : rawExtension;
  const options = resolveMkdocsMarkdownOptions(config.adapterOptions);
