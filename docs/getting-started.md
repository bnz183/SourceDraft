...
Sign in with `SOURCEDRAFT_ADMIN_PASSWORD` — this is the local Studio password, not your GitHub login.

On first visit, the sign-in screen shows five starting paths. **Try demo mode** if you are unsure; it needs no GitHub or API tokens.
...
2. Fill title and description in the center canvas; set slug, dates, category, and tags in **Article details**; upload a cover image if needed ([media.md](media.md))
